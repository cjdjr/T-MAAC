...
class CSTRANSTRANSMADDPG(Model):
    ...
    def encode(self, raw_obs, raw_act, merge_act = False):
        ...
        obs, _ = self.encoder(th.cat((obs,zone_id),dim=-1), None, agent_index, mask, final_mask)
    ...
    def encode(self, raw_obs):
        ...
        mask = self.adj_mask[None,:,:,:].repeat(batch_size,1,1,1).view(batch_size*self.n_, self.obs_bus_num, self.obs_bus_num).contiguous()
        final_mask = self.obs_mask[None,:,None,:].repeat(batch_size,1,1,1).view(batch_size*self.n_,1,-1).contiguous() # (b*n, 1, obs_bus_num)
        # final_mask = self.obs_flag[None,:,:, None].repeat(batch_size,1,1,1).view(batch_size*self.n_,self.obs_bus_num,1).contiguous() # (b*n, num, 1)
        # final_mask = th.cat((mask, th.zeros(batch_size*self.n_, 1, 1).float().to(self.device)),dim = -1) # (b*n, 1, obs_bus_num +1 ) for hidden_state
        agent_index = th.tensor(self.agent_index_in_obs)[None,:,None].repeat(batch_size, 1, 1).view(batch_size*self.n_, 1).contiguous().to(self.device)
