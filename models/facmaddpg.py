...
class FACMADDPG(Model):
    ...
    def get_loss(self, batch):
        ...
        values_pol = self.value(state, actions_pol).contiguous(
        ).view(-1, self.n_)  # shape = (b, n)
        # get the q_tot
        values = self.value(state, actions).contiguous(
        ).view(-1, self.n_)  # shape = (b, n)
        global_state = state.contiguous().view(
            batch_size, self.n_*self.obs_dim)  # shape = (b, n*o)
        q_tot = self.mixer(values, global_state).contiguous(
        ).view(-1, 1)  # shape = (b, 1)
        # get the next q_tot
        next_values = self.target_net.value(next_state, next_actions.detach(
        )).contiguous().view(-1, self.n_)  # shape = (b, n)
        next_global_state = next_state.contiguous().view(
            batch_size, self.n_*self.obs_dim)  # shape = (b, n*o)
        next_q_tot = self.target_net.mixer(
            next_values, next_global_state).contiguous().view(-1, 1)  # shape = (b, 1)
        returns = th.zeros_like(q_tot).to(self.device)
        assert q_tot.size() == next_q_tot.size()
        assert returns.size() == q_tot.size()
        done = done.to(self.device)
        returns = rewards[:, 0:1] + self.args.gamma * \
            (1 - done) * next_q_tot.detach()
