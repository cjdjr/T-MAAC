...
class IAC(Model):
    ...
    def get_actions(self, state, status, exploration, actions_avail, target=False, last_hid=None):
        ...
        if self.args.continuous:
            means, log_stds, hid = self.policy(
            ...
            restore_mask = 1. - \
                cuda_wrapper((actions_avail == 0).float(), self.cuda_)
            restore_actions = restore_mask * actions
            action_out = (means, log_stds)
        else:
            logits, _, hid = self.policy(
                state) if not target else target_policy(state, last_hid=last_hid)
    ...
    def get_loss(self, batch):
        policy_loss, value_loss, action_out = self.rl.get_loss(
            batch, self, self.target_net)
