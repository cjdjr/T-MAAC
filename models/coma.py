...
class COMA(Model):
    ...
    def construct_value_net(self):
        if self.args.continuous:
            if self.args.agent_id:
                input_shape = (self.n_ + 1) * self.obs_dim + \
                    self.n_ * self.act_dim + self.n_
            else:
                input_shape = (self.n_ + 1) * self.obs_dim + \
                    self.n_ * self.act_dim
            output_shape = 1
        else:
            if self.args.agent_id:
                input_shape = (self.n_ + 1) * self.obs_dim + \
    ...
    def value(self, obs, act):
        ...
        if gaussian_flag:
            ...
            obs_own = obs.clone()
            obs_own = obs_own.unsqueeze(0).expand(self.sample_size, batch_size, self.n_, self.obs_dim).contiguous(
            ).view(-1, self.n_, self.obs_dim)  # (s*b, n, o)
            # shape = (b, n, o) -> (1, b, 1, n, o) -> (s, b, n, n, o)
            obs = obs.unsqueeze(0).unsqueeze(2).expand(
                self.sample_size, batch_size, self.n_, self.n_, self.obs_dim)
            obs = obs.contiguous().view(-1, self.n_, self.n_*self.obs_dim)  # (s*b, n, n*o)
            inp = th.cat((obs, obs_own), dim=-1)  # shape = (s*b, n, o*n+o)
        else:
            batch_size = obs.size(0)
            obs_own = obs.clone()
            # shape = (b, n, o) -> (b, 1, n, o) -> (b, n, n, o)
            obs = obs.unsqueeze(1).expand(
                batch_size, self.n_, self.n_, self.obs_dim)
        ...
        if self.args.agent_id:
            agent_ids = th.eye(self.n_).unsqueeze(0).repeat(
                inp.size(0), 1, 1).to(self.device)  # shape = (b/s*b, n, n)
            # shape = (b/s*b, n, o*n+o+n)
            inp = th.cat((inp, agent_ids), dim=-1)
            if self.args.shared_params:
                inp = inp.contiguous().view(-1, self.obs_dim*(self.n_+1) +
                                            self.n_)  # shape = (b*n/s*b*n, o*n+o+n)
        else:
            if self.args.shared_params:
                inp = inp.contiguous().view(-1, self.obs_dim*(self.n_+1)
                                            )  # shape = (b*n/s*b*n, o*n+o)
        ...
        if self.args.continuous:
            if gaussian_flag:
                ...
                act_reshape = act.unsqueeze(1).repeat(
                    1, self.n_, 1, 1)  # shape = (b, n, n, a)
                if self.args.shared_params:
                    act_reshape = act_reshape.contiguous().view(-1, self.n_*self.act_dim)
                else:
                    act_reshape = act_reshape.contiguous().view(batch_size, self.n_, self.n_ *
                                                                self.act_dim)  # shape = (b, n, n*a)
                inputs = th.cat((inp, act_reshape), dim=-1)
        else:
            # other agents' actions
            act_repeat = act.unsqueeze(1).repeat(
                1, self.n_, 1, 1)  # shape = (b, n, n, a)
            agent_mask = th.eye(self.n_).unsqueeze(
                0).unsqueeze(-1).expand_as(act_repeat).to(self.device)  # shape = (b, n, n, a)
            agent_mask_complement = 1. - agent_mask
            act_mask_out = agent_mask_complement * \
                act_repeat  # shape = (b, n, n, a)
            if self.args.shared_params:
                act_other = act_mask_out.contiguous().view(-1, self.n_ *
                                                           self.act_dim)  # shape = (b*n, n*a)
            else:
                act_other = act_mask_out.contiguous().view(batch_size, self.n_, self.n_ *
                                                           self.act_dim)  # shape = (b, n, n*a)
    ...
    def get_loss(self, batch):
        ...
        if self.args.continuous:
            ...
            means_repeat = means.unsqueeze(0).repeat(
                self.sample_size, 1, 1, 1)  # (s,b,n,a)
            log_stds_repeat = log_stds.unsqueeze(0).repeat(
                self.sample_size, 1, 1, 1)  # (s,b,n,a)
            _sampled_actions_repeat = th.normal(
                means_repeat, log_stds_repeat.exp())  # (s,b,n,a)
            sampled_actions_repeat = _sampled_actions_repeat.unsqueeze(
                2).repeat(1, 1, self.n_, 1, 1)  # (s,b,n,a) -> (s,b,n,n,a)
            actions_repeat = actions.unsqueeze(0).unsqueeze(2).repeat(
                self.sample_size, 1, self.n_, 1, 1)  # (s,b,n,n,a)
            agent_mask = th.eye(self.n_).unsqueeze(0).unsqueeze(
                1).unsqueeze(-1).expand_as(actions_repeat).to(self.device)  # (s,b,n,n,a)
            agent_mask_complement = 1. - agent_mask
            actions_repeat_merge = actions_repeat * agent_mask_complement + \
                sampled_actions_repeat * agent_mask  # (s,b,n,n,a)
            actions_repeat_merge = actions_repeat_merge.contiguous(
            ).view(-1, self.n_, self.n_*self.act_dim)  # (s*b,n,n*a)
            values_sampled = self.value(state, actions_repeat_merge).contiguous().view(
                self.sample_size, batch_size, self.n_)  # (s*b,n,1) -> (s,b,n)
            baselines = th.mean(values_sampled, dim=0)  # (b,n)
            # (b,n,a) -> (b,n) action value
            values = self.value(state, actions).squeeze(-1)
        else:
            logits, _, _ = self.policy(state, last_hid=last_hids)
            action_out = logits
            ...
            # the only difference to ActorCritic is this  baseline (b,n)
            baselines = th.sum(values*th.softmax(logits, dim=-1), dim=-1)
        ...
        ), f"log_prob size is: {log_prob_a.size()} and advantages size is {advantages.size()}."
