...
class SQDDPG(Model):
    ...
    def sample_grandcoalitions(self, batch_size):
        seq_set = th.tril(th.ones(self.n_, self.n_),
                          diagonal=0, out=None).to(self.device)
        grand_coalitions = th.multinomial(th.ones(
            batch_size*self.sample_size, self.n_)/self.n_, self.n_, replacement=False).to(self.device)
        individual_map = th.zeros(
            batch_size*self.sample_size*self.n_, self.n_).to(self.device)
        individual_map.scatter_(
            1, grand_coalitions.contiguous().view(-1, 1), 1)
        ...
        grand_coalitions = grand_coalitions.unsqueeze(1).expand(batch_size*self.sample_size, self.n_, self.n_).contiguous(
        ).view(batch_size, self.sample_size, self.n_, self.n_)  # shape = (b, n_s, n, n)
        return subcoalition_map, grand_coalitions, individual_map

    def marginal_contribution(self, obs, act):
        batch_size = obs.size(0)
        subcoalition_map, grand_coalitions, individual_map = self.sample_grandcoalitions(
            batch_size)  # shape = (b, n_s, n, n)
        grand_coalitions_expand = grand_coalitions.unsqueeze(-1).expand(
            batch_size, self.sample_size, self.n_, self.n_, self.act_dim)  # shape = (b, n_s, n, n, a)
        act = act.unsqueeze(1).unsqueeze(2).expand(batch_size, self.sample_size, self.n_, self.n_, self.act_dim).gather(
            3, grand_coalitions_expand)  # shape = (b, n, a) -> (b, 1, 1, n, a) -> (b, n_s, n, n, a)
        subcoalition_map_no_i = subcoalition_map - \
            individual_map  # shape = (b, n_s, n, n)
        # shape = (b, n_s, n, n, 1)
        subcoalition_map_no_i = subcoalition_map_no_i.unsqueeze(-1)
        # shape = (b, n_s, n, n, 1)
        individual_map = individual_map.unsqueeze(-1)
        ...
        # make up inputs
        act = act.contiguous().view(batch_size, self.sample_size,
                                    self.n_, -1)  # shape = (b, n_s, n, n*a)
        ...
        obs = obs.contiguous().view(batch_size, self.sample_size, self.n_, self.n_ *
                                    self.obs_dim)  # shape = (b, n_s, n, n, o) -> (b, n_s, n, n*o)
        inp = th.cat((obs, act), dim=-1)  # shape = (b, n_s, n, n*o+n*a)

        inp = inp.contiguous().view(batch_size*self.sample_size,
                                    self.n_, -1)  # shape = (b*n_s, n, n*o+n*a)

        # add agent id
        if self.args.agent_id:
            agent_ids = th.eye(self.n_).unsqueeze(0).repeat(
                batch_size*self.sample_size, 1, 1).to(self.device)  # shape = (b*n_s, n, n)
            # shape = (b*n_s, n, n*o+n*a+n)
            inp = th.cat((inp, agent_ids), dim=-1)

        if self.args.shared_params:
            inputs = inp.contiguous().view(batch_size*self.sample_size*self.n_, -
                                           1)  # shape = (b*n_s*n, n*o+n*a/n*o+n*a+n)
        ...
        values = values.contiguous().view(batch_size, self.sample_size,
                                          self.n_, 1)  # shape = (b, n_s, n, 1)
    ...
    def get_actions(self, state, status, exploration, actions_avail, target=False, last_hid=None):
        ...
        if self.args.continuous:
            ...
            actions, log_prob_a = select_action(
                self.args, means_, status=status, exploration=exploration, info={'log_std': log_stds_})
    ...
    def get_loss(self, batch):
        ...
        shapley_values_pol = self.marginal_contribution(
            state, actions_pol).mean(dim=1).contiguous().view(-1, self.n_)
        # do the exploration action on the value loss
        shapley_values_sum = self.marginal_contribution(state, actions).mean(dim=1).contiguous(
        ).view(-1, self.n_).sum(dim=-1, keepdim=True).expand(batch_size, self.n_)
        if self.args.target:
            next_shapley_values_sum = self.target_net.marginal_contribution(next_state, next_actions.detach()).mean(
                dim=1).contiguous().view(-1, self.n_).sum(dim=-1, keepdim=True).expand(batch_size, self.n_)
        else:
            next_shapley_values_sum = self.marginal_contribution(next_state, next_actions.detach()).mean(
                dim=1).contiguous().view(-1, self.n_).sum(dim=-1, keepdim=True).expand(batch_size, self.n_)
        ...
        returns = rewards + self.args.gamma * \
            (1 - done) * next_shapley_values_sum.detach()
