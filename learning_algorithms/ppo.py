...
class PPO(ReinforcementLearning):
    ...
    def get_loss(self, batch, behaviour_net, target_net):
        ...
        if self.args.continuous:
            means, log_stds, _ = behaviour_net.policy(
                state, last_hid=last_hids)
            if means.size(-1) > 1:
        ...
        ratios = th.exp(log_prob_a.squeeze(-1) -
                        old_log_prob_a.squeeze(-1).detach())
        values = behaviour_net.value(
            state, None).contiguous().view(-1, behaviour_net.n_)
        next_values = behaviour_net.value(
            next_state, None).contiguous().view(-1, behaviour_net.n_)
        advantages = th.zeros((batch_size, behaviour_net.n_),
                              dtype=th.float).to(self.device)
        returns = th.zeros((batch_size, behaviour_net.n_),
                           dtype=th.float).to(self.device)
        ...
        for i in reversed(range(rewards.size(0))):
            if last_step[i]:
                ...
                mask = 1.0
            deltas_ = rewards[i] + behaviour_net.args.gamma * \
                old_next_values[i].detach() * mask - old_values[i]
            last_advantages = deltas_ + behaviour_net.args.gamma * \
                behaviour_net.args.lambda_ * last_advantages * mask
            advantages[i] = last_advantages
        done = done.to(self.device)
        returns = rewards + behaviour_net.args.gamma * \
            (1 - done) * next_values.detach()
        # normalise advantage
        ...
        surr2 = th.clamp(ratios, 1 - behaviour_net.args.eps_clip,
                         1 + behaviour_net.args.eps_clip) * advantages.detach()
        policy_loss = - th.min(surr1, surr2).mean()
        # value loss
        assert old_values.size() == values.size()
        values_clipped = old_values + \
            th.clamp(values - old_values, - behaviour_net.args.eps_clip,
